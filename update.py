...
for x in range(20,120,20):        
    try:
        with open('mono@'+str(x)+'%.gif','r')as r:
            r.close()
    except:
        r=urllib.request.urlopen('https://raw.githubusercontent.com/jellybeanstaffy/frogmatic/frogmatic/mono@'+str(x)+'%.gif')
        with open('mono@'+str(x)+'%.gif','wb')as w:
            w.write(r.read())
            w.close()
